...
defaultProtocolData topic = mconcat
  ...
  , build16 5 -- protocol name length
  , buildArray (fromByteString "range") 5
  , build32 16 -- metadata bytes length
  , build16 0 -- version
  , build32 1 -- number of subscriptions
  , build16 (size16 topic) -- size of topic name
  , buildArray topic (sizeofByteArray topic) -- topic name
